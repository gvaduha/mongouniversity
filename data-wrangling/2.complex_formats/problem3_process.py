    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in table.find_all('tr'):
            if (header):
                header = False
                continue
            year, month, domn, intn, _ = [col.text for col in row.find_all('td')]
            # skip last
            if 'TOTAL' in month: continue
            info["year"] = int(year)
            info["month"] = int(month)
            info["flights"] = {"domestic":int(str(domn).translate(None, ',')), "international":int(str(intn).translate(None, ','))}
...
def test():
    print "\n[ATTENTION]: data file IS NOT correct for these tests\n"
