...
def extract_carriers(page):
    ...
    with open(page, "r") as html:
        ...
        selbox = soup.find(id="CarrierList")

	## Parse for select/options texts
        #data = str(selbox.text).strip('\n').split('\n')
        ## cleaning 'All '
        #data = filter( lambda x : 'All ' not in x, data)

        # Parse for select/options values
        data = [x['value'] for x in selbox.find_all('option') if len(x['value']) == 2]
...
def make_request(data):
    """
    !!! ! It's correct to use Session instead of request to preserve session cookies
    """
