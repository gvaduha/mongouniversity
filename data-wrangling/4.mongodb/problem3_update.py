...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
	    label = re.sub("(.*)\(.*\)", r'\1', line['rdf-schema#label']).strip()
	    binau = line['binomialAuthority_label']
            if binau != 'NULL':
                data[label] = binau
...
def update_db(data, db):
    ...
    """
    for item in data:
        db.arachnid.update({'label': item}, {'$set': {'classification.binomialAuthority': data[item]} })
