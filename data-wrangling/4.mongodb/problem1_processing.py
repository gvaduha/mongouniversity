...
def raw_item(line, fields):
    item = {}
    item['classification'] = {}
    for f in fields:
        val = None if line[f] == 'NULL' else line[f].strip()
        if re.search('.*_label', f):
            item['classification'][fields[f]] = val
        else:
            item[fields[f]] = val
    #print item
    return item
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
	    item = raw_item(line, fields)
            item['label'] = re.sub("(.*)\(.*\)", r'\1', item['label']).strip()
            if item['name'] is None or not item['name'].isalnum(): item['name'] = item['label']
            if item['synonym'] is not None:
                if item['synonym'][0] == '{' and item['synonym'][-1] == '}':
                    item['synonym'] = item['synonym'].strip('{}').split('|')
                    item['synonym'] = map(lambda x: x.strip('*').strip(), item['synonym'])
                else:
                    item['synonym'] = [item['synonym'].strip('*').strip()]
